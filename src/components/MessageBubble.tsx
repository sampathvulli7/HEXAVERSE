...
export const MessageBubble = memo(function MessageBubble({ message, onCitationClick, onFollowUpClick }: MessageBubbleProps) {
  ...
  const [charIndex, setCharIndex] = useState(shouldStream ? 0 : message.content.length);
  const intervalRef = useRef<ReturnType<typeof setInterval> | null>(null);

  // Streaming: ~40ms interval, 3-4 chars per tick (same perceived speed, ~60% fewer re-renders than 18ms/3-5)
  useEffect(() => {
    if (!shouldStream || charIndex >= message.content.length) return;
  ...
  }, [shouldStream, charIndex, message.content.length]);

  const isDoneStreaming = charIndex >= message.content.length;
  const visibleText = isUser ? message.content : message.content.slice(0, charIndex);
