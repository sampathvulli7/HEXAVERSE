...
import { Plus, FolderKanban, FolderPlus, X, Sun, Moon, ChevronDown, Search, FileText, Image as ImageIcon, Music, PanelLeftClose, PanelLeftOpen } from 'lucide-react';
...
export function Sidebar({ 
  ...
  const [projects, setProjects] = useState([
    { id: 'p1', title: 'SIH Multimodal Intelligence', count: 12, color: 'bg-blue-500' },
    { id: 'p2', title: 'Document Analytics Suite', count: 8, color: 'bg-purple-500' },
    { id: 'p3', title: 'Audio & Speech Transcripts', count: 5, color: 'bg-emerald-500' },
  ]);
            ...
            {(projectsExpanded || isCollapsed) && (
              ...
              >
                {filteredProjects.map((proj) => {
                  ...
                  const files = PROJECT_FILES[proj.id] || [];
                  ...
                  if (isCollapsed) {
                    return (
                      <button
                        ...
                        className="w-10 h-10 rounded-full bg-gray-100 dark:bg-zinc-800/80 hover:bg-gray-200 dark:hover:bg-zinc-700 flex items-center justify-center transition-colors"
                      >
                        <FolderKanban className={cn("w-4 h-4", proj.color.replace('bg-', 'text-'))} />
                      </button>
                    );
                  }
                  ...
                  return (
                    <div key={proj.id}>
                      <button
                        onClick={() => setExpandedProjectId(isExpanded ? null : proj.id)}
                        className="flex items-center gap-2 w-full px-2 py-1.5 rounded-lg text-left text-gray-700 dark:text-gray-300 bg-transparent hover:bg-gray-50/80 dark:hover:bg-zinc-800/50 border border-transparent hover:border-gray-200/50 dark:hover:border-zinc-700/50 transition-all group"
                      >
                        <div className={cn("w-2 h-2 rounded-full flex-shrink-0", proj.color)} />
                        <div className="flex flex-col flex-1 min-w-0">
                          <span className="text-xs font-medium truncate">{proj.title}</span>
                          <span className="text-[9px] text-gray-400 dark:text-gray-500">{proj.count} files</span>
                        </div>
                        {files.length > 0 && (
                          <span className="text-[9px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-1.5 py-0.5 rounded-md flex-shrink-0">
                            {indexedCount}/{files.length}
                          </span>
                        )}
                        <motion.div
                          animate={{ rotate: isExpanded ? 180 : 0 }}
                          className="flex-shrink-0 text-gray-300 dark:text-gray-600 group-hover:text-gray-500"
                        >
                      ...
                      <AnimatePresence>
                        {isExpanded && files.length > 0 && (
                          <motion.div
                            ...
                            className="overflow-hidden"
                          >
                            <div className="ml-4 pl-2 border-l border-gray-200/60 dark:border-zinc-800/60 py-1 space-y-0.5 mt-1">
                              {files.map((file, i) => (
                                <div key={i} className="flex items-center gap-2 px-1.5 py-1 rounded-md text-[10px] text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors">
